...
pub extern "system" fn Java_com_hydra_jni_RAMStoreJNI_put(
    ...
    let job_id_str: String = env.get_string(&job_id).expect("Couldn't get job ID").into();
    ...
    env.get_byte_array_region(&checksum, 0, &mut checksum_bytes)
        .unwrap();
    let checksum_bytes_u8: Vec<u8> = checksum_bytes.iter().map(|&b| b as u8).collect();

    let store = STORE.read().unwrap();
    if let Some(ref s) = *store {
        let checksum_array: [u8; 32] = checksum_bytes_u8.try_into().unwrap_or([0u8; 32]);
        let result = s.put(
            &job_id_str,
            chunk_index as u32,
            data_bytes_u8,
...
pub extern "system" fn Java_com_hydra_jni_RAMStoreJNI_delete(
    ...
    if let Some(ref mut s) = *store {
        if s.delete(&job_id_str, chunk_index as u32) {
            1
        } else {
            0
        }
...
#[no_mangle]
pub extern "system" fn Java_com_hydra_jni_RAMStoreJNI_snapshot(_env: JNIEnv, _class: JClass) {
    let store = STORE.read().unwrap();
    if let Some(ref s) = *store {
        s.snapshot();
    }
...
#[no_mangle]
pub extern "system" fn Java_com_hydra_jni_RAMStoreJNI_restore(_env: JNIEnv, _class: JClass) {
    let mut store = STORE.write().unwrap();
    if let Some(ref mut s) = *store {
        s.restore();
    }
